...
class egMatLibPanel(QWidget):
    ...
    def check_add_category(self, cat):
        ...
        if not cat in self.config['categories']:
            entry = {cat: ""}
    ...
    def save_material(self):
        ...
        choice, cat = hou.ui.readInput("Add Object to Category")
        if choice:
            return
        ...
        if self.save_node(sel[0]):
            # Add Material to our Library
            self.addMaterialToLibrary(sel[0], cat, tags)
        self.save_config()
